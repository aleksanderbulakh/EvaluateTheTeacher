...
namespace AvaluateTheTeacher1.CodeReview.Models
{
    public class VotingModel
    {
        ...
        public VotingList InfoForRaitingList(int? groupId)
        {
            using (var db = new ApplicationDbContext())
            {
                var thisGroup = db.Groups.Find(groupId);

                var r = (from listTeachersSubject in thisGroup.TeachersSubjects
                         join listSubject in db.Subjects on listTeachersSubject.SubjectId equals listSubject.Id
                         join listTeachers in db.Teachers on listTeachersSubject.TeacherId equals listTeachers.TeacherId
                         join listRaiting in db.Ratings on listTeachers.TeacherId equals listRaiting.TeacherId
                         orderby listRaiting.AvgRating descending
                         select new ListOfTeachers
                         {
